class NumberBaseConverter {
    ...
    init() {
        const inputs = document.querySelectorAll('#decimal-input, #binary-input, #octal-input, #hex-input');
    ...
    updateAllFields(decimalValue, sourceInputId) {
        ...
        if (sourceInputId !== 'hex-input') {
            document.getElementById('hex-input').value = decimalValue.toString(16).toUpperCase();
        }
    }

    clearOtherInputs(sourceInputId) {
        const inputs = ['decimal-input', 'binary-input', 'octal-input', 'hex-input'];
    ...
    showError(inputId, message) {
        ...
        if (!errorSpan) {
            ...
            errorSpan.style.cssText = 'color: #dc3545; font-size: 12px; display: block; margin-top: 5px;';
